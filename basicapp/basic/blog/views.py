...
get_allcategories = Category.objects.all()
get_allposts = Post.objects.published()
AllMonths = { 1 : 'Jan' , 2 : 'Feb' , 3 : 'Mar' , 4 : 'Apr' , 5 : 'May', 6 : 'Jun' , 7 : 'Jul',8 : 'Aug',9 : 'Sep',10 : 'Oct',11 : 'Nov',12 : 'Dec',}
AllPublishedMonths = []

def get_allpublishedmonths() :
	AllPublishedMonths = []
	for post in get_allposts :
		AllPublishedMonths.append((AllMonths[post.publish.month],post.publish.year))
	return set(AllPublishedMonths)
...
def post_list(request, page=0, **kwargs):        
	...
	extra_context = { 'category_list' : get_allcategories, 'months' : get_allpublishedmonths},
        paginate_by = 20,
        page = page,
        **kwargs
    )
...
def posted(request, **kwargs) :
	return render_to_response('blog/posted.html',{ 'category_list' : get_allcategories, 'months' : get_allpublishedmonths() })

def preview(request, **kwargs) :
	return render_to_response('blog/preview.html',{ 'category_list' : get_allcategories, 'months' : get_allpublishedmonths() })
...
def category_detail(request, slug, template_name = 'blog/category_detail.html', **kwargs):
    ...
    return list_detail.object_list(
        ...
        extra_context = {'category': category ,'category_list' : get_allcategories, 'months' : get_allpublishedmonths},
        template_name = template_name,
        **kwargs
    )
